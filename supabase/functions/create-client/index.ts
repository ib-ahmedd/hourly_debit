...
Deno.serve(async (req) => {
  ...
  const { user_id, name, balance, hourly_debit_amount } = await req.json();
  const { error } = await supabase
    .from("customers")
    .insert([
      { user_id, name, balance, hourly_debit_amount },
    ]);
  ...
  if (!error) {
    ...
    return new Response(
      JSON.stringify({ message: "Failed to create customer.", error: error }),
      {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      },
